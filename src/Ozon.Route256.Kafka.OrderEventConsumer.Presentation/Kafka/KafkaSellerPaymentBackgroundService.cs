...
public class KafkaSellerPaymentBackgroundService : BackgroundService
{
    private readonly KafkaAsyncConsumer<Ignore, OrderEvent> _consumer;
    private readonly ILogger<KafkaSellerPaymentBackgroundService> _logger;

    public KafkaSellerPaymentBackgroundService(IServiceProvider serviceProvider, ILogger<KafkaSellerPaymentBackgroundService> logger)
    {
        // TODO: IOptions
        // TODO: KafkaServiceExtensions: services.AddKafkaHandler<TKey, TValue, THandler<TKey, TValue>>(serializers, topic, groupId);
        _logger = logger;
        var handler = serviceProvider.GetRequiredService<SellerPaymentHandler>();
